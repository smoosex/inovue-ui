...
const baseRgb = ref({ r: 0, g: 0, b: 0 });
const activeRgb = ref({ r: 0, g: 0, b: 0 });
...
const parseColor = (color: string) => {
  ...
  // Append to body to resolve CSS variables if needed, though for var() it usually needs to be in the DOM tree where the variable is defined.
  // However, creating a detached canvas might not resolve var(--primary) correctly if it depends on :root.
  // Let's try using the existing canvasRef if available, or a temp one attached to wrapper.

  ctx.fillStyle = color;
  // If color is a variable, we need the context of the document.
  // A detached canvas usually can't resolve CSS variables unless we getComputedStyle first.
  // But we can't getComputedStyle on a color string.
  // Trick: Create a temp element, set color, getComputedStyle.
...
const updateColors = () => {
  baseRgb.value = parseColor(props.baseColor);
  activeRgb.value = parseColor(props.activeColor);
};
